...
if(isset($_POST['login'])){
    ...
    if(count($errors) == 0){
        $chkUser = mysqli_query($dbConn, "SELECT * FROM members WHERE (memberID='$username' OR username='$username' OR email='$username') AND password='$cryptPwd'");
        if(mysqli_num_rows($chkUser) == 1){
            $memberData = mysqli_fetch_array($chkUser);
            $ID = $memberData['memberID'];
            $status = $memberData['status'];
            if($status == "Active"){
                $_SESSION['ck_Member'] = $ID;
                $_SESSION['authMsg'] = "Welcome back. Pick from where you left";
                header("location: home?shw-pend=member");
            }
            else{
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <form class="row g-3" action="" method="post">
                                        ...
                                        <div class="col-12">
                                            <p class="small mb-0">Don't have account? <a href="<?php $membersURL; ?>register">Sign up</a></p>
                                        </div>
